...
A 2.153-point EER advantage between two Speech DF Arena detectors loses simultaneous-band separation when speakers and attacks replace trials as the resampling unit. The same change occurs for XLSR-Mamba versus XLSR-Conformer, despite substantial cancellation from system pairing. With thresholds refitted and the comparison family held fixed, trial and joint resampling separate 26/28 and 18/28 pairs on primary ASVspoof~2021~DF; a separate four-detector SpoofCeleb roster changes from 6/6 to 3/6. Speaker-only perturbations match the joint separation indicators on the primary roster, and attack-only perturbations do so on SpoofCeleb. Measured sensitivity concentrates in particular observed groups, while all SSL-versus-baseline comparisons survive the tested perturbations. Paired comparisons can therefore remain sensitive to group perturbations after substantial common variation cancels.
\end{abstract}
...
\textbf{Surviving modern comparisons.} Of the two within-SSL pairs separated by joint resampling, Mamba--SSL-AASIST loses separation when VCC2018 is omitted (gap $-0.968\to-0.270$ points); SLS--SSL-AASIST loses it when VCC2020 is omitted ($-0.935\to-0.760$). Source deletion uses a third construction, a marginal-floor jackknife with Gaussian max-$t$ calibration over all 28 pairs (S2), related to max-se methods \cite{mackinnon24}; it preserves all 16 cross-cohort separations. The first loss combines gap shrinkage with the changed band; the second retains a sizeable gap but gains uncertainty.
...
\textbf{Coverage boundary.} In the low-EER simulations, all three pointwise candidate intervals had coverage below .90 in the same 11/24 cells (S7). These imposed-model simulations evaluate different intervals from the principal all-pair bands and do not establish a population sampling model for 21DF.

\begingroup\raggedright
\textbf{Artifact.} Supplement S4a--S4e supplies the complete four-arm bands, Monte Carlo repeats, mean/sign and pairing diagnostics, weighted-tie checks and group influence. The three evidence JSON files and analysis code accompany artifact version \texttt{m1-impact-20260916-v1} at \texttt{github.com/\allowbreak rvirgilli/\allowbreak asvspoof2021-\allowbreak df-clustered-\allowbreak uncertainty}; S8 gives file selectors and hashes.\par\endgroup

\section{Conclusion}
Trial-resampling separation can disappear under speaker--attack perturbations for both Arena HuBERT--WavLM and primary Mamba--Conformer, despite paired cancellation. Speaker-only matches the joint indicators on primary 21DF; attack-only does so on the separate SpoofCeleb roster, where trial-to-joint separation falls from 6/6 to 3/6. Cross-cohort separation survives the tested laws, weights and source deletions, while modern within-cohort comparisons expose concentrated group sensitivity.
