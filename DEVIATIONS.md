...
| The 5/6-to-0/6 matched contrast also moved source/task composition | EXP-111 added a post-failure conditioned arm that restores every class-by-source/task mass | It retains 0/6 organizer and 18/28 overall endpoints, addressing that confound; the result was known before the amendment and is not called prospective. |
| The organizer-like coverage aggregate lacked a witnessed trace | EXP-112 replaced it with a clean, hash-bound $R=200$ trace and independent reconstruction | The paper uses 18.5% versus 97.5--98.0% with Wilson intervals, conditional on the fitted Gaussian DGP; DGP adequacy for 21DF remains unaudited. |
| No independent corpus isolated sampling-unit movement from source mixture | EXP-114 was frozen before SpoofCeleb access and gave 6/6 trial-i.i.d. versus 3/6 product/source exclusions | A later audit found the original scorer entrypoint absent from the seal. A disclosed post-result clean rerun reproduced three score files exactly and the fourth within $1.44\times10^{-6}$ conservatively rounded upward, leaving all EERs, decisions and the endpoint unchanged. |
| EXP-114 did not isolate speaker and spoof-attack perturbations | EXP-115 froze two factor-only arms after the joint endpoint was known, then gave 5/6 speaker-only and 3/6 attack-only exclusions | Attack-only exactly reproduces the joint-product verdict vector, while speaker-only is sufficient for one changed decision. This is a disclosed post-result mechanism diagnostic, not prospective confirmation, causal variance decomposition or population inference. |
